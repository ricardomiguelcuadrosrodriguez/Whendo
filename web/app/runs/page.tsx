      ...
      <section className="mx-auto max-w-6xl px-6 py-10">
        <div className="flex items-end justify-between gap-3">
          <div>
            ...
            <p className="mt-1 font-mono text-2xl text-ink">
              ...
              <span className="text-ink-dim">executions</span>
            </p>
          </div>
          <button
            onClick={fetchRuns}
        ...
        {loadState === "ready" && runs.length === 0 && (
          <div className="mt-10 border border-border bg-bg-card p-10 text-center">
            <p className="font-mono text-sm text-ink-dim">No runs yet.</p>
            <p className="mt-3 font-mono text-xs text-ink-muted">
              Recipes will execute when their schedule fires, or run them
              manually from the recipes page.
            </p>
          </div>
        )}
  ...
  return (
    <>
      ...
      {expanded && (
        ...
        >
          <KV k="status" v={<span className={statusColor}>{run.status}</span>} />
          <KV k="started_at" v={run.started_at} />
          <KV k="finished_at" v={run.finished_at ?? "—"} />
          {run.error && (
            <KV
              k="error"
